...
import { Container, Table, Button, Form, Spinner, Toast, Modal } from 'react-bootstrap';
import { getAdminAllUsers, deleteAdminUser, putAdminUserRole } from '../../api/admins';
...
const Users = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setToastMessage(error.response ? error.response.data.message : 'Fehler beim Laden der Benutzer');
  ...
  const handleDelete = async () => {
    if (userIdToDelete) {
      try {
        ...
        setToastMessage(error.response ? error.response.data.message : 'Fehler beim Löschen des Benutzers');
        setToastVariant('danger');
        setShowToast(true);
      } finally {
  ...
  const handleRoleChange = async (userId, newRole) => {
    try {
      ...
      setToastMessage(response.message || `Rolle für Benutzer ${userId} erfolgreich geändert`); // Nachricht aus der Response
      setToastVariant('success');
      setShowToast(true);
      fetchUsers(); // Benutzerliste nach dem Rollenwechsel aktualisieren
    } catch (error) {
      // Hier wird die Fehlermeldung vom Backend verwendet
      setToastMessage(error.response ? error.response.data.message : 'Fehler beim Ändern der Rolle');
      setToastVariant('danger');
      setShowToast(true);
    }
  };
  ...
  return (
    <Container className="mt-5 mb-5">
      <h5 className='headline text-success mt-3 mb-3 text-lg-start'>Benutzer verwalten</h5>
      ...
      >
        <Toast.Header>
          <strong className="me-auto text-${toastVariant}">Benachrichtigung</strong>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} className='bg-mordal'>
        <Modal.Header closeButton>
          <Modal.Title className='text-color headline2'>Benutzer löschen</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          Möchten Sie den Benutzer mit der{' '}
          <span className="fw-bold">ID {userIdToDelete}</span> wirklich
          löschen?
        </Modal.Body>
        <Modal.Footer>
